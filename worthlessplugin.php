...
?>
	<div class="wrap">
	  	...
	  	<form method="post" action="<?php echo $_SERVER['REQUEST_URI'];?>">
		  	<?php wp_nonce_field('worthless_save','worthless_nonce'); ?>
...
function worthless_deactivate() {
	delete_option('worthless_config');
}
...
function worthless_options_submit() {
	if(wp_verify_nonce($_POST['worthless_nonce'], 'worthless_save')) {
		$buffer = get_option('worthless_config');
		$seed = $_POST['worthless_saved'];
		if(!is_numeric($seed)) $seed = 300;
		...
		echo '	<h2 style="text-align: center;">Oh no! Something went wrong!</h2>';
		echo '	<p style="text-align: center;">WordPress was unable to verify the authenticity of the url you have clicked. Verify if the url used is valid or log in via your browser.</p>';
		echo '	<p style="text-align: center;">If you have received the url you want to visit via email, you are being tricked!</p>';
		exit;
	}
}
?>